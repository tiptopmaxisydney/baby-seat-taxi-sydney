 ...
 * adapted for baby seat/child seat bookings: step navigation, per-step
 * validation, and the Google Places autocomplete + service-area gating.
 * The submit step posts straight to the TipTop Ride dispatch backend's
 * quote endpoint (see wcbConfig.ts).
 */
...
export function attachBookingForm(form: HTMLFormElement, config: WCBConfig): () => void {
  ...
  function warn(message: string) {
    if (window.console?.warn) window.console.warn("[Baby Seat Booking] " + message);
  }
  ...
  function handleSubmit(event: Event) {
    ...
    // The API has separate counters for capsules and child seats but no
    // field for "booster seat" or a passenger name, so the exact seat type
    // and any passenger name are folded into notes rather than dropped.
    const seatType = (data.get("seatType") as string) || "";
    const seatCount = parseInt((data.get("seatCount") as string) || "1", 10);
    const passengerName = (data.get("passengerName") as string) || "";
    const driverInstructions = (data.get("driverInstructions") as string) || "";
    const notes = [
      seatType && `Seat type requested: ${seatType} x${seatCount}`,
    ...
    // Baby capsules and child/booster seats map to the two counters the
    // backend understands; a booster seat is booked as a child seat.
    const isCapsule = seatType.toLowerCase().includes("capsule");
    ...
    const payload = {
      ...
      no_of_wheelchair: 0,
      no_of_childseat: noOfChildSeat,
      no_of_childcapsule: noOfChildCapsule,
