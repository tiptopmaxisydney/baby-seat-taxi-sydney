...
const BookingWizard: React.FC<BookingWizardProps> = ({
  ...
  // wheelchair count left over from a previously selected vehicle so it doesn't keep
  // affecting the passenger/luggage caps or the fare summary while the field is hidden.
  useEffect(() => {
    if (/suv/i.test(vehicleInfo?.vehicle_name || "") && form2.getFieldValue("no_of_wheelchair")) {
      form2.setFieldValue("no_of_wheelchair", 0);
    }
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-6">
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
        <button
          ...
          className="rounded-full px-5 py-2 text-sm font-semibold border transition-colors border-[#1d3649] text-[#1d3649] hover:bg-[#1d3649]/5 disabled:border-slate-200 disabled:text-slate-300 disabled:cursor-not-allowed disabled:hover:bg-transparent"
        >
          Back
        </button>
        {currentStep < 4 ? (
          <button
            ...
            className="rounded-full px-6 py-2 text-sm font-semibold bg-[#f78d1e] text-white transition-colors hover:bg-[#ff3802] disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-[#f78d1e]"
          >
            Next Step
          </button>
        ) : (
