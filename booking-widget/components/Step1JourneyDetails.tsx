    ...
    <>
      <div className="mb-3">
        ...
        {isAirportPickup && (
          <div className="mb-3">
            ...
            <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 h-[44px] transition-colors focus-within:border-[#1d3649] focus-within:ring-2 focus-within:ring-[#1d3649]/10">
        ...
        {stops.map((stop, index) => (
          <div key={index} className="flex items-center gap-2 mt-3">
            <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 h-[44px] flex-1 transition-colors focus-within:border-[#1d3649] focus-within:ring-2 focus-within:ring-[#1d3649]/10">
              <span className="flex-shrink-0 flex items-center">
                <RadioIcon />
              </span>
              <Form.Item
        ...
        <button
          type="button"
          className="mt-3 flex items-center gap-2 text-sm text-[#1d3649] font-medium hover:opacity-80"
      ...
      {isReturnTrip && (
        <div className="mb-3 border rounded-xl bg-slate-50 p-4 mt-3">
          <h3 className="fw-semibold mb-3 text-[#1d3649]">Return Trip Details</h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Pickup Location</label>
              <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 h-[44px] bg-white transition-colors focus-within:border-[#1d3649] focus-within:ring-2 focus-within:ring-[#1d3649]/10">
                <span className="flex-shrink-0 flex items-center">
                  <RadioIcon />
                </span>
                <Form.Item
          ...
          {returnStops.map((stop, index) => (
            <div key={index} className="flex items-center gap-2 mt-3">
              <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 h-[44px] bg-white flex-1 transition-colors focus-within:border-[#1d3649] focus-within:ring-2 focus-within:ring-[#1d3649]/10">
                <span className="flex-shrink-0 flex items-center">
                  <RadioIcon />
                </span>
                <Form.Item
      ...
      <Modal
        ...
        title={
          <div className="text-center pt-1">
            ...
            <h2 className="mt-3 mb-0 text-lg font-semibold">Select your pickup point</h2>
          </div>
        }
      >
        <div className="mt-4 mb-2">
          <RadioGroup
            ...
            onChange={(value) => setSelectedAddress(value.target.value)}
          ...
          >
            {
              country == "in" ? <>
                <Radio
                  className={[
                    "!flex !w-full !items-start !mr-0 gap-2 rounded-xl border px-4 py-3 transition-colors",
                    selectedAddress === "mohali_international_airport"
                      ? "!border-2 border-[#1d3649] bg-[#1d3649]/[0.03]"
                      : "border-slate-200 hover:border-[#1d3649]/40",
                  ].join(" ")}
                  value="mohali_international_airport"
                >
                  <span className="text-sm text-[#33383f]">Mohali International Airport - Pickup from Rideshare Area</span>
                </Radio>
              </> : <>
                <Radio
                  className={[
                    "!flex !w-full !items-start !mr-0 gap-2 rounded-xl border px-4 py-3 transition-colors",
                    selectedAddress === "Sydney_international_airport"
                      ? "!border-2 border-[#1d3649] bg-[#1d3649]/[0.03]"
                      : "border-slate-200 hover:border-[#1d3649]/40",
                  ].join(" ")}
                  value="Sydney_international_airport"
                >
                  <span className="text-sm text-[#33383f]">Rideshare Area (Near P7 Buildings) Sydney International Airport</span>
                </Radio>
                <Radio
                  className={[
                    "!flex !w-full !items-start !mr-0 gap-2 rounded-xl border px-4 py-3 transition-colors",
                    selectedAddress === "sydney_domestic_airport"
                      ? "!border-2 border-[#1d3649] bg-[#1d3649]/[0.03]"
                      : "border-slate-200 hover:border-[#1d3649]/40",
                  ].join(" ")}
                  value="sydney_domestic_airport"
                >
                  <span className="text-sm text-[#33383f]">Green Priority Pickup Area (T2, T3), Sydney Domestic Airport.</span>
                </Radio>
              </>
            }
          </RadioGroup>
        ...
        <div className="flex items-center justify-end gap-3 mt-5 pt-4 border-t border-slate-100">
          <button
            type="button"
            onClick={() => setIsAirport(false)}
            className="rounded-full px-5 py-2 text-sm font-semibold border border-[#1d3649] text-[#1d3649] hover:bg-[#1d3649]/5 transition-colors"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={onSelectmodalAddress}
            className="rounded-full px-6 py-2 text-sm font-semibold bg-[#f78d1e] text-white hover:bg-[#c9720a] transition-colors"
          >
            Confirm
          </button>
        </div>
