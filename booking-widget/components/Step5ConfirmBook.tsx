...
const SummaryRow: React.FC<{ label: string; value?: React.ReactNode; full?: boolean }> = ({ label, value, full }) =>
  value === undefined || value === null || value === "" ? null : (
    <div className={full ? "sm:col-span-2" : ""}>
      ...
      <div className="font-medium text-[#1d3649] break-words">{value}</div>
    </div>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start mb-4">
        <div className="w-full flex flex-col rounded-xl border border-slate-200 p-4">
          <h4 className="fw-semibold mb-2 text-[#1d3649]">Trip details</h4>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
          ...
          {babySeatItems ? (
            <SummaryRow
              label="Seat types"
              value={babySeatItems
                .map((item) => `${getBabySeatType(item.seat_type)?.label} (${getBabySeatType(item.seat_type)?.hint})`)
                .join(", ")}
              full
            />
          ) : (
      ...
      <div className="w-full flex flex-col rounded-xl border border-slate-200 p-4">
        <h4 className="fw-semibold mb-2 text-[#1d3649]">Amount details</h4>
